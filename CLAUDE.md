...
## ⚠️ MANDATORY: Root Cause Analysis (RCA) при ошибках

**При ЛЮБОЙ допущенной ошибке** ОБЯЗАТЕЛЬНО провести RCA методом "5 почему":

### Процесс RCA

```
Ошибка → СТОП → 5 Почему → Фиксация → Обновление инструкций → Продолжение
```

### Метод "5 Почему"

Каждый ответ становится базой для следующего вопроса:

| Уровень | Вопрос | Пример |
|---------|--------|--------|
| 1 | Почему [ошибка]? | Почему контейнер недоступен? → Traefik не находит маршрут |
| 2 | Почему [ответ1]? | Почему Traefik не находит? → Контейнер в другой сети |
| 3 | Почему [ответ2]? | Почему в другой сети? → Конфиг не совпадает с сервером |
| 4 | Почему [ответ3]? | Почему не совпадает? → Нет валидации перед деплоем |
| 5 | Почему [ответ4]? | Почему нет валидации? → Отсутствует preflight-check для сетей |

**Корневая причина** = ответ на 5-й уровень (системная, изменяемая)

### После RCA ОБЯЗАТЕЛЬНО:

1. **Зафиксировать** в `docs/rca/YYYY-MM-DD-[topic].md` (используй шаблон `docs/rca/TEMPLATE.md`)
2. **Оценить обновления:**
   - [ ] CLAUDE.md — новое правило/предостережение?
   - [ ] MEMORY.md — новая конфигурация/pitfall?
   - [ ] Новый навык — если ошибка повторяющаяся?
   - [ ] Существующий навык — расширить чеклист?
...
Или вручную следуй процессу из `.claude/skills/rca-5-whys/SKILL.md`

### Принципы RCA

- **Не искать виноватых** — искать системные причины
- **Не останавливаться на симптоме** — доходить до корня
- **Не повторять ошибок** — обновлять инструкции
