...
### Авто-триггеры для ошибок LLM (саморефлексия обязательна)

RCA запускается автоматически, если:
1. Любая команда завершилась с `exit code != 0`
2. Пользователь явно указывает на ошибку понимания (`"ты не понял"`, `"это не то"`, `"ошибка"`)
3. Обнаружен drift контекста (не та ветка/worktree/состояние)

При срабатывании триггера сначала выдать блок саморефлексии, затем продолжать:
