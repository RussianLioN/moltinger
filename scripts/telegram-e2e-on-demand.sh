...
reply_has_codex_update_scheduler_memory_false_negative() {
  ...
  case "$normalized" in
    *"проверить по памяти/расписанию"*|*"инструмент поиска памяти"*|*"не вижу подтверждения, что такой крон"*|*"подтвердить наличие такого крона я сейчас не могу"*|*"в памяти у меня явно записано"*|*"по сохранённой памяти ответ однозначный"*|*"по сохраненной памяти ответ однозначный"*|*"по сохранённой памяти у меня зафиксировано"*|*"по сохраненной памяти у меня зафиксировано"*|*"ежедневно проверяю стабильные обновления codex cli"*|*"ежедневно проверять стабильные обновления codex cli"*|*"searching memory"*)
      return 0
      ;;
  esac
