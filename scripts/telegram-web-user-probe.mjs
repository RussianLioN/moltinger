...
const INTERNAL_PLANNING_RE =
  /(?:пользователь просит|the user (?:is )?asking|у меня есть доступ к|i have access to|мне доступны|сначала найду|для начала найду|(?:(?:отлично|супер|окей|ладно)[!,. ]{0,12})?давай(?:те)?\s*(?:получу|найду|изучу|посмотрю|открою|проверю|проанализирую)|хорошо,?\s*(?:изучу|проверю|посмотрю|почитаю).{0,120}(?:документац|docs|documentation|manual|guide|инструкц)|начну с (?:поиска|анализа|изучения|просмотра)|наш[её]л.{0,120}(?:репозитор|github|документац|docs|documentation|manual|guide|инструкц)|получ(?:у|им|ить).{0,120}(?:документац|docs|documentation|manual|guide|инструкц)|mounted workspace|skill files|existing skills|существующ(?:ие|его) навык|имеющ(?:егося|ийся) навы|mcp__[\p{L}\p{N}_:.-]+)/iu;
