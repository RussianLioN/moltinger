...
| https://docs.openclaw.ai/models-and-auth | 2026-03-09 | OpenClaw has provider auth flows, including `openai-codex`, and supports stored auth profiles. | Clawdiy should treat OpenAI Codex auth as its own lifecycle with operator verification, not as a shared host credential. |
...
| https://github.com/openclaw/openclaw/issues/39994 | 2026-03-09 | `openai-codex` OAuth can succeed superficially but still lack required scopes such as `api.responses.write`. | OpenAI Codex auth cannot be assumed healthy after login; the plan needs explicit post-auth verification and fail-closed behavior. |
