...
5. Treat current production as still running the stock `origin/main` browser contract until proven otherwise; if authoritative inspection shows `sandbox_image = "browserless/chrome"` without the tracked `profile_dir` / `persist_profile` contract, the remediation path is a minimal browser hotfix carrier to `main` followed by a canonical deploy from `main`.
...
1. freeze the audited browser root cause and official-docs verdict in tracked artifacts
2. materialize a minimal browser-runtime carrier for `main`
3. validate that carrier hermetically against clean `origin/main`
4. only then use the canonical deploy path from `main` for the real live browser repair
