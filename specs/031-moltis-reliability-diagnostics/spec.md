...
- What happens when stock `browserless/chrome` is already healthy on the target host once the profile bind is writable, so the safest hotfix is an official-first profile-dir repair instead of a custom browser image?
...
- **FR-028**: When live production still runs the pre-fix browser baseline from `main`, the package MUST prepare a minimal browser-runtime carrier to `main` rather than assuming the `031` branch alone closes the incident; the carrier scope must stay limited to production-critical browser contract files plus blocking validation.
...
- **SC-016**: The package leaves behind a browser-runtime carrier artifact for `main` that matches the audited live root cause and does not mix mutable docs/process layers into the production-critical browser rollout.
