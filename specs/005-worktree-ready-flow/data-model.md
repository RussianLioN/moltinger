...
- **Purpose**: Represents the deterministic pre-mutation decision for one-shot start flows.
- **Fields**:
  - `slug`: normalized slug inferred from user input
  - `issue_id`: optional issue reference
  - `branch_name`: derived clean or existing branch target
  - `path_preview`: user-facing relative sibling path
  - `worktree_path`: absolute candidate path
  - `decision`: `create_clean` | `attach_existing_branch` | `reuse_existing` | `needs_clarification`
  - `topology_state`: `ok` | `stale` | `unavailable`
  - `question`: optional one-line clarification prompt
...
- **Purpose**: Describes one existing line that is similar enough to a slug-only request to justify a clarification question.
- **Fields**:
  - `candidate_type`: `worktree` | `local-branch` | `remote-branch`
  - `name`: branch or worktree name shown to the user
  - `path`: optional absolute path for an attached worktree
  - `reason`: short classifier such as `exact-attached-branch`, `similar-local-branch`, `exact-remote-branch`
