...
JSON
    ...
    if jq '. + {
      ...
      "brief_section_updates": {
        ...
        assert_eq "1" "$(jq -r '.brief_feedback_history | length' "$tmpdir/brief-revision-out.json")" "Each correction should be persisted in feedback history"
        assert_eq "applied" "$(jq -r '.brief_feedback_history[-1].status' "$tmpdir/brief-revision-out.json")" "Applied correction feedback should be marked as applied"
        assert_eq "1.0" "$(jq -r '.brief_feedback_history[-1].source_brief_version' "$tmpdir/brief-revision-out.json")" "Feedback must point to the reviewed source version"
        assert_eq "1.1" "$(jq -r '.brief_feedback_history[-1].applied_in_brief_version' "$tmpdir/brief-revision-out.json")" "Feedback must be linked to the new generated version"
