...
run_component_agent_factory_web_delivery_tests() {
    ...
    if python3 "$WEB_ADAPTER_SCRIPT" handle-turn --source "$DELIVERY_FIXTURE" --state-root "$tmpdir/state" --output "$tmpdir/downloads-out.json" >/dev/null; then
        ...
        assert_eq "3" "$(jq -r '.download_artifacts | length' "$tmpdir/downloads-out.json")" "Three concept-pack artifacts should be exposed to the browser"
        assert_eq "3" "$(jq -r '[.download_artifacts[] | select(.download_status == "ready")] | length' "$tmpdir/downloads-out.json")" "All browser downloads should be marked ready"
