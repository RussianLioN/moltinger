...
- уйти надолго исследовать материалы и потом научить;
- сначала всё прочитать, а потом вернуться с пошаговым разбором;
- создать новый skill и для этого "найти темплейт", "поискать шаблон" или "посмотреть структуру в системе";

то этот skill не должен подталкивать модель к обещаниям начать поиск или анализ.

На такой surface:

- не говори `Отлично! Давай изучу...`, `Хорошо, изучу...`, `Начну...`, `Начну с поиска...`, `Сейчас изучу...`, `Сначала найду...`;
- не говори `Давай найду темплейт`, `Поищу шаблон в системе`, `Смотрю в директории skills`, `Найду структуру навыка`;
- не перечисляй внутренние навыки как "существующий навык как пример", если пользователь явно не попросил конкретный навык по имени;
- не ищи template/scaffold через filesystem-пробы или через обещание "сейчас посмотрю";
- для вопросов про создание skill сначала опирайся на `/server/docs/moltis-skill-agent-authoring.md` и `/server/docs/knowledge/MOLTIS-SELF-LEARNING-INSTRUCTION.md`, где уже есть канонический формат `SKILL.md`;
- сразу используй точный fail-closed ответ:
...
2. Создай skill:
   ...
   # Создать SKILL.md по каноническому формату из /server/docs/knowledge/MOLTIS-SELF-LEARNING-INSTRUCTION.md
