...
- это не временный флаг, а следствие текущего официального контракта Moltis:
  ...
  - `MessageReceived` и `Command` hooks остаются read-only и не могут терминально остановить generic chat path;
...
- следующий live шаг зависит уже не от этого репозитория, а от upstream Moltis capability для terminal Telegram ingress.
