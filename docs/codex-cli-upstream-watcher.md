...
- shared consent store закрывает запрос как `declined`;
- второе сообщение с рекомендациями не отправляется;
- повторный `decline` не должен ничего менять кроме audit trail.

Если пользователь повторяет `accept` после уже обработанного согласия:

- router отвечает, что запрос уже был обработан;
- второй пакет рекомендаций повторно не уходит;
- store остаётся в безопасном idempotent-состоянии.

Простыми словами:

- `accept` = рекомендации приходят сразу;
- `decline` = диалог закрывается без follow-up;
- duplicate `accept` = без спама и без второго дубля рекомендаций.

## Что делать оператору, если follow-up не доставился

Shared consent store теперь фиксирует и состояние второй отправки:

- `sent`: рекомендации успешно доставлены;
- `suppressed`: пользователь отказался, follow-up не нужен;
- `retry`: согласие принято, но отправка рекомендаций не завершилась и нужен повтор;
- `failed`: отправка сломана и требует явной проверки конфигурации или транспорта.

Практически это означает:

- если transport доступен, пользователь получает рекомендации сразу;
- если router видит согласие, но sender недоступен или Telegram-отправка падает, запрос не теряется;
- оператор может увидеть `retry/failed` в store/report и повторить доставку осознанно, а не гадать, ушло сообщение или нет.

## Acceptance path для feature 017

Для acceptance-проверки сценария `alert -> consent -> recommendations` теперь есть отдельный helper:
...
- это не просто unit или component check;
- это короткий сквозной proof, что UX больше не зависит от старого watcher-side polling пути.

## Rollout

Безопасный порядок включения такой:

1. убедиться, что доступны `moltis-codex-consent-router.sh` и `codex-telegram-consent-store.sh`;
2. проверить, что watcher видит router как `consent_router_ready=true`;
3. прогнать `make codex-consent-e2e`;
4. только потом оставлять consent question включённым в scheduler-уведомлениях.
...
Если authoritative router временно нельзя держать включённым, rollback простой:

- отключить router path через `CODEX_UPSTREAM_WATCHER_TELEGRAM_CONSENT_ROUTER_ENABLED=false`
