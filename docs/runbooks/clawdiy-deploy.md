...
3. Sync repo-managed artifacts through CI/CD or GitOps deploy flow.
   ...
   If the only extra drift is stray `config/oauth_tokens.json`, treat it as a known runtime OAuth artifact: the repair path now snapshots it and evacuates it into `data/oauth-config/` before cleaning the checkout.
4. Deploy Clawdiy:
   ```bash
