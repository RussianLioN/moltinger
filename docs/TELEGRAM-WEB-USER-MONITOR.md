...
Дополнительно authoritative probe использует `reply settle window`: verdict строится не по первому входящему сообщению, а по последнему стабильному ответу после короткого окна тишины. Это защищает от ложных pass, когда бот сначала пишет промежуточное `Проверяю...`, а затем завершает ответ timeout-ошибкой.
