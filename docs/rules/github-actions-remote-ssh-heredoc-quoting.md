...
4. For Moltis deploy control-plane mutations, the current authoritative entrypoints are `scripts/align-server-checkout.sh`, `scripts/render-moltis-env.sh`, `scripts/gitops-sync-managed-surface.sh`, `scripts/update-active-deploy-root.sh`, `scripts/apply-moltis-host-automation.sh`, and `scripts/run-tracked-moltis-deploy.sh`.
