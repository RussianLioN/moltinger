...
| `MOLTINGER_SERVICE_TOKEN` | Internal fleet bearer auth | Moltis env + fleet handoff hooks |
...
For Moltis Telegram ingress:
- authoritative runtime allowlist lives in tracked `config/moltis.toml` under `[channels.telegram.moltis-bot].allowlist`
- CI derives `TELEGRAM_ALLOWED_USERS` in `/opt/moltinger/.env` from that tracked config for auxiliary scripts only
- do not treat a GitHub secret named `TELEGRAM_ALLOWED_USERS` as the runtime source of truth for Moltis auth
...
The same deploy flow also renders `/opt/moltinger/.env` with:

- `MOLTIS_PASSWORD`
- `MOLTINGER_SERVICE_TOKEN`
- provider/search secrets
- `TELEGRAM_ALLOWED_USERS` mirrored from tracked `config/moltis.toml`
